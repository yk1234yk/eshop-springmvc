      ...
      <div class="col-md-6 mt-3 mt-md-0">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb justify-content-md-end bg-transparent p-0 m-0">
            ...
            <li class="breadcrumb-item"><a class="link-title" href="category?method=all">全部分类</a></li>
          ...
          <ul class="pagination justify-content-center">
          	<c:choose>
          		<c:when test="${page.pageIndex == 1}">
          			<li class="page-item">
          				<a class="page-link" href="category?method=list&pageSize=${page.pageSize}&pageIndex=${page.pageIndex}&id=${id}">前一页</a></li>
          		</c:when>
            	<c:otherwise>
            		<li class="page-item">
            			<a class="page-link" href="category?method=list&pageSize=${page.pageSize}&pageIndex=${page.pageIndex-1}&id=${id}">前一页</a></li>
            	</c:otherwise>
            </c:choose>
            <c:forEach begin="1" end="${page.totalPage}" varStatus="status">
            	<li class="page-item">
            		<a class="page-link" href="category?method=list&pageSize=${page.pageSize}&pageIndex=${status.index}&id=${id}">${status.index}</a></li>
            </c:forEach>
            <c:choose>
          		<c:when test="${page.pageIndex == page.totalPage}">
          			<li class="page-item"><a class="page-link" href="category?method=list&pageSize=${page.pageSize}&pageIndex=${page.pageIndex}&id=${id}">后一页</a></li>
          		</c:when>
            	<c:otherwise>
            		<li class="page-item"><a class="page-link" href="category?method=list&pageSize=${page.pageSize}&pageIndex=${page.pageIndex+1}&id=${id}">后一页</a></li>
            	</c:otherwise>
            </c:choose>
          </ul>
        </nav>
