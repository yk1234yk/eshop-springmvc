                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb justify-content-md-end bg-transparent p-0 m-0">
                            ...
                            <li class="breadcrumb-item"><a class="link-title"
                                                           href="${pageContext.request.contextPath}/category/all">全部分类商品</a>
                            </li>
                            ...
                            <ul class="pagination justify-content-center">
                                <c:choose>
                                    <c:when test="${page.pageIndex == 1}">
                                        <li class="page-item">
                                            <a class="page-link"
                                               href="${pageContext.request.contextPath}/category/search?pageSize=${page.pageSize}&pageIndex=${page.pageIndex}&name=${name}&brief=${brief}&detail=${detail}">前一页</a>
                                        </li>
                                    </c:when>
                                    <c:otherwise>
                                        <li class="page-item">
                                            <a class="page-link"
                                               href="${pageContext.request.contextPath}/category/search?pageSize=${page.pageSize}&pageIndex=${page.pageIndex-1}&name=${name}&brief=${brief}&detail=${detail}">前一页</a>
                                        </li>
                                    </c:otherwise>
                                </c:choose>
                                <c:forEach begin="1" end="${page.totalPage}" varStatus="status">
                                    <li class="page-item">
                                        <a class="page-link"
                                           href="${pageContext.request.contextPath}/category/search?pageSize=${page.pageSize}&pageIndex=${status.index}&name=${name}&brief=${brief}&detail=${detail}">${status.index}</a>
                                    </li>
                                </c:forEach>
                                <c:choose>
                                    <c:when test="${page.pageIndex == page.totalPage}">
                                        <li class="page-item"><a class="page-link"
                                                                 href="${pageContext.request.contextPath}/category/search?pageSize=${page.pageSize}&pageIndex=${page.pageIndex}&name=${name}&brief=${brief}&detail=${detail}">后一页</a>
                                        </li>
                                    </c:when>
                                    <c:otherwise>
                                        <li class="page-item"><a class="page-link"
                                                                 href="${pageContext.request.contextPath}/category/search?pageSize=${page.pageSize}&pageIndex=${page.pageIndex+1}&name=${name}&brief=${brief}&detail=${detail}">后一页</a>
                                        </li>
                                    </c:otherwise>
                                </c:choose>
                            </ul>
                                                ...
                                                <div class="card-body text-muted">
                                                    <ul class="list-unstyled">
                                                        <c:forEach var="childCategory" items="${topCategory.children}">
                                                            <li>
                                                                <a href="${pageContext.request.contextPath}/category/list/${childCategory.id}">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${childCategory.name}</a>
                                                            </li>
                                                        </c:forEach>
                                                    </ul>
                                                </div>
